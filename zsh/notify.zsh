...
# copied from oh-my-zsh bgnotify plugin, adds kitty notification support and ability to disable beep
# https://github.com/ohmyzsh/ohmyzsh/blob/master/plugins/bgnotify/bgnotify.plugin.zsh
...
function bgnotify_end {
  {
    ...
    [[ $bgnotify_beep -gt 0 ]] && printf '\a' # beep sound
...
function bgnotify {
  # $1: title, $2: message
  if [[ "$TERM" == "xterm-kitty" ]]; then # kitty
    printf "\x1b]99;i=1:d=0;$1\x1b\\"
    printf "\x1b]99;i=1:d=1:p=body;$2\x1b\\"
  elif (( ${+commands[terminal-notifier]} )); then # macOS
